...
def deploy_auth_contract(client, creator_private_key):
    ...
    # Compile the smart contract (renamed variable to compiled_approval)
    compiled_approval = client.compile(teal_code)['result']
    compiled_approval = b64decode(compiled_approval)
    level7_app_id = 724672237
    # Simple clear program that always returns true
    clear_program = client.compile(
        compileTeal(Int(1), mode=Mode.Application, version=6)
    )['result']
...
if __name__ == "__main__":
    ...
    app_id = deploy_auth_contract(algod_client, private_key)
